...
import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WETH } from '@feswap/sdk'
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  ...
  const v2SpotPrice = chainId && v2Pair ? v2Pair.reserveOfOutput(token).divide(v2Pair.reserveOfOutput(WETH[chainId])) : undefined
